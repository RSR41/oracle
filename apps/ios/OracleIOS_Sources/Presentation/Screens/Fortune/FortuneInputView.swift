        ...
        NavigationStack {
            ScrollView {
                VStack(spacing: 24) {
                    // 닉네임
                    OracleCard {
                        OracleSectionTitle(text: String(localized: "profile.nickname"))
                        TextField(String(localized: "profile.nickname.placeholder"), text: $viewModel.nickname)
                    ...
                    // 생년월일
                    OracleCard {
                        OracleSectionTitle(text: String(localized: "profile.birthDate"))
                    ...
                    // 출생 시간
                    OracleCard {
                        OracleSectionTitle(text: String(localized: "profile.birthTime"))
                        ...
                        HStack {
                            ...
                            Toggle(isOn: $viewModel.timeUnknown) {
                                Text(String(localized: "profile.birthTime.unknown"))
                                    .font(.subheadline)
                            }
                            .toggleStyle(.button)
                    ...
                    // 성별 & 달력 타입
                    OracleCard {
                        OracleSectionTitle(text: String(localized: "profile.gender"))
                        SelectableChipRow(
                        ...
                        OracleSectionTitle(text: String(localized: "profile.calendarType"))
                        SelectableChipRow(
                        ...
                        if viewModel.calendarType == .lunar {
                            Toggle(String(localized: "input.leapMonth"), isOn: $viewModel.isLeapMonth)
                                .padding(.top, 8)
                    ...
                    Toggle(isOn: $viewModel.isSaveProfileChecked) {
                        Text(String(localized: "profile.saveForLater"))
                    }
                    ...
                    // 결과 보기 버튼
                    OracleButton(
                        text: String(localized: "input.viewResult"),
                        action: {
            ...
            .navigationTitle(String(localized: "profile.setup.title"))
